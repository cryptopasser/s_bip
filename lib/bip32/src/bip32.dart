...
import 'utils/ecurve_util.dart' as ecurve;
import 'utils/wif_util.dart' as wif;
...
class BIP32 {
  ...
  Uint8List get publicKey {
    if (_Q == null) _Q = ecurve.pointFromScalar(_d!, true)!;
    return _Q!;
  }
  ...
  sign(Uint8List hash) {
    return ecurve.sign(hash, privateKey!);
  }

  verify(Uint8List hash, Uint8List signature) {
    return ecurve.verify(hash, publicKey, signature);
  }
  ...
  factory BIP32.fromPrivateKey(Uint8List privateKey, Uint8List chainCode,
    ...
    if (!ecurve.isPrivate(privateKey))
